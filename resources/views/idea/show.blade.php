<x-layout>
    <div class="py-8 max-w-4xl mx-auto">
        <div class="flex justify-between">
            <a href="{{route('idea.index')}}" class="flex item-center gap-x-2 text-sm font-medium">
                <x-icons.arrow-back />
                Back to Ideas
            </a>
        ...
        <div class="mt-8 space-y-6">
            @if($idea->image_path)
                <div class="rounded-lg overflow-hidden">
                    <img src="{{ asset('storage/' . $idea->image_path) }}" alt="Image Idea" class="w-full h-auto object-cover">
                </div>
            @endif
            ...
            @if($idea->description)
                <x-card class="mt-6" is="div">
                    <div class="text-foreground max-w-none cursor-pointer prose-invert">{!!$idea->formattedDescription!!}</div>
                </x-card>
            @endif
